 ...
 * 生成财务预测（基于规划和资产数据）
 */
export async function generateForecast(userId: string, planId: string): Promise<Forecast | null> {
  // 1. 获取规划数据
...
export async function deleteForecast(forecastId: string): Promise<boolean> {
  const supabase = await createClient();
  // 1. 删除预测详情
  const { error: detailsError } = await supabase.from(Tables.ForecastDetails).delete().eq('forecast_id', forecastId);
  ...
  // 2. 删除预测主记录
  const { error } = await supabase.from(Tables.Forecasts).delete().eq('id', forecastId);
