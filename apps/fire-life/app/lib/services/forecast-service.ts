...
export const forecastService = {
   ...
   * 创建新预测
   */
  async createForecast(forecast: Omit<Forecast, 'id' | 'created_at'>, details: Omit<ForecastDetail, 'id' | 'forecast_id'>[]): Promise<Forecast | null> {
    // 1. 创建预测主记录
    const { data, error } = await supabase
      .from(Tables.Forecasts)
      .insert({
   ...
   * 生成财务预测（基于规划和资产数据）
   */
  async generateForecast(userId: string, planId: string): Promise<Forecast | null> {
    // 1. 获取规划数据
