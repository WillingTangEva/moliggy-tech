...
export const planService = {
   ...
   * 创建新财务规划
   */
  async createPlan(plan: Omit<FinancialPlan, 'id' | 'created_at' | 'updated_at'>): Promise<FinancialPlan | null> {
    const { data, error } = await supabase
      .from(Tables.FinancialPlans)
      .insert({
        ...plan,
   ...
   * 更新财务规划
   */
  async updatePlan(id: string, updates: Partial<Omit<FinancialPlan, 'id' | 'user_id' | 'created_at'>>): Promise<FinancialPlan | null> {
    const { data, error } = await supabase
      .from(Tables.FinancialPlans)
      .update({
        ...updates,
