 ...
 * 按类型获取用户资产
 */
export async function getUserAssetsByType(userId: string, type: AssetType): Promise<Asset[]> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from(Tables.Assets)
    .select('*')
 ...
 * 获取用户资产总值
 */
export async function getUserTotalAssetValue(userId: string, currency: Currency = 'CNY'): Promise<number> {
 ...
 * 获取资产分布（按类型）
 */
export async function getAssetDistribution(userId: string): Promise<Record<AssetType, number>> {
  const assets = await getUserAssets(userId);
...
export async function getAllAssets(): Promise<Asset[]> {
  ...
  const { data, error } = await supabase.from(Tables.Assets).select('*').order('created_at', { ascending: false });
