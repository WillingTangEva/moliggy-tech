...
function calculateReadinessScore(
  ...
  // 预计退休时的资产（基于当前储蓄率）
  const projectedAssets = calculateProjectedAssets(currentAssets, annualSavings, yearsToRetirement);
...
export function calculateForecast(
  plan: FinancialPlan,
  currentAssets: number
): { forecast: Omit<Forecast, 'id' | 'created_at' | 'user_id' | 'plan_id'>, details: Omit<ForecastDetail, 'id' | 'forecast_id'>[] } {
  // 1. 计算关键参数
  const withdrawalRate = getSafeWithdrawalRate(plan.risk_tolerance);
  const annualExpenses = plan.monthly_expenses * 12;
  const requiredRetirementAssets = calculateRequiredRetirementAssets(annualExpenses, withdrawalRate);
  ...
  // 4. 检查是否可以提前退休
  for (let testAge = plan.current_age; testAge <= plan.target_retirement_age; testAge++) {
