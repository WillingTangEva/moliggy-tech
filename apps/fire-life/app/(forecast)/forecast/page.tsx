      ...
      <div className="container mx-auto py-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">基于您的财务数据，预测未来财务状况和退休收入</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>您需要创建一个财务计划才能进行退休预测</CardDescription>
          </CardHeader>
          <CardContent className="pb-6 text-center">
            <p className="mb-4">您还没有创建任何财务计划。请先创建一个财务计划，然后再进行退休预测。</p>
            <Button onClick={() => router.push('/plans/new')}>创建财务计划</Button>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="container mx-auto py-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleCreateForecast} disabled={calculating || !selectedPlanId}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
              ...
              {plans.length === 0 && (
                <div className="mt-2">
                  <p className="text-muted-foreground mb-2 text-sm">您需要先创建一个财务计划才能进行预测</p>
                  <Button variant="outline" size="sm" onClick={() => router.push('/plans/new')}>
                    创建财务计划
                  </Button>
                </div>
              )}
            ...
            <div>
              ...
              <div className="relative mt-1">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 transform">¥</span>
                <input
                  id="current-assets"
                  type="number"
                  value={currentAssets}
      ...
      <div className="grid gap-8">
              ...
              {retirementResult ? (
                <>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-xs font-medium text-green-600">{retirementResult.readinessScore}%</span>
                  </div>
                  <p className="text-muted-foreground mt-1 text-xs">
                    {new Date().getFullYear() + (retirementResult.actualRetirementAge - currentAge)}年
                  </p>
                </>
              ) : (
            ...
            <CardContent>
              {retirementResult ? (
                <>
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">当前: ¥{currentAssets.toLocaleString()}</p>
                </>
              ) : (
                <div className="flex h-14 items-center justify-center">
                  <p className="text-muted-foreground text-sm">选择计划来查看</p>
            ...
            <CardContent>
              {retirementResult ? (
                <>
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">退休后每月可用支出</p>
                </>
              ) : (
                <div className="flex h-14 items-center justify-center">
                  <p className="text-muted-foreground text-sm">选择计划来查看</p>
        ...
        {retirementResult && retirementResult.yearlyDetails && (
          <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div>
              <RetirementReadinessCard {...readinessCardProps} />
            </div>
          </div>
        )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>调整参数，查看不同情景下的退休预测结果</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-8">
              <div className="grid gap-6">
                <div className="space-y-2">
                  ...
                  <Slider
                    id="inflation"
                    value={[inflationRate]}
                    onValueChange={(values) => setInflationRate(values[0] || 3)}
                    max={10}
                    step={0.1}
                    className="w-full"
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    id="return"
                    value={[returnRate]}
                    onValueChange={(values) => setReturnRate(values[0] || 7)}
                    max={15}
                    step={0.1}
                    className="w-full"
                  />
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button onClick={applyParameterChanges} disabled={calculating || !selectedPlanId}>
